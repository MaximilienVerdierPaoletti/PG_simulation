...
def create_configuration_sidebar():
    ...
    if uploaded_images:
        ...
        st.sidebar.success(f"{len(uploaded_images)} file(s) uploaded")
    else:
        st.session_state.uploaded_image_files = []
        st.sidebar.info("No image files uploaded")
    ...
    if uploaded_data:
        ...
        st.sidebar.success(f"Data file: {uploaded_data.name}")
    else:
        st.session_state.uploaded_data_file = None
        st.sidebar.info("No data file uploaded")

    st.sidebar.markdown("---")

    # Grain Parameters Section
    st.sidebar.markdown("#### Grain Parameters")

    st.session_state.nb_pg = st.sidebar.number_input(
    ...
    st.session_state.nb_closest_match = st.sidebar.number_input(
    ...
    st.session_state.beam = st.sidebar.number_input(
        "Beam Size:",
        min_value=1,
        max_value=1000,
        value=100,
    ...
    st.session_state.boxcar = st.sidebar.number_input(
        "Boxcar Pixels:",
        min_value=1,
        max_value=20,
        value=3,
    ...
    st.session_state.elem = st.sidebar.text_input(
        "Element:",
        value="O",
        key="elem_input",
    )
    ...
    st.sidebar.markdown("#### Delta Database Ranges")

    delta_defaults = [
        ("Range 1", -900, 0, 50),
        ("Range 2", 0, 200, 10),
        ("Range 3", 200, 2000, 100),
        ("Range 4", 2000, 21000, 1000),
    ]

    for i, (range_name, start_def, stop_def, step_def) in enumerate(
        delta_defaults, 1
    ):
        st.sidebar.markdown(f"**{range_name}:**")
    ...
    st.sidebar.markdown("#### Iteration Parameters")

    st.session_state.iterations = st.sidebar.number_input(
        "Outer Iterations:",
        min_value=1,
        max_value=100,
        value=1,
    ...
    st.session_state.max_iteration = st.sidebar.number_input(
    ...
    st.session_state.cost_goal = st.sidebar.number_input(
        "Cost Goal:",
        min_value=0.01,
        max_value=10.0,
        value=0.4,
    ...
    # Size Range Section
    st.sidebar.markdown("#### Size Range Parameters")
    ...
    st.session_state.save_all_plots = st.sidebar.checkbox(
        "Save All Plots",
        value=False,
        key="save_all_plots_input",
    )

    st.session_state.enable_profiling = st.sidebar.checkbox(
        "Enable Profiling",
        value=False,
        key="enable_profiling_input",
    )

    st.session_state.profiling_dir = st.sidebar.text_input(
...
def update_original_data_graph():
    ...
    if st.session_state.simulation_results["f_OG"] is not None:
        ...
        st.info("No original grain figure (f_OG) available yet. Run simulation to see original grain data.")
...
def update_simulations_graph():
    ...
    if summary is not None:
        ...
        st.info("No simulation data available yet. Run simulation to see all simulations.")
...
def run_simulation():
    ...
    try:
        ...
        normalized_file_list = [f.replace("\\", "/") for f in st.session_state.file_list]
    ...
    def update_progress():
        ...
        status_text.text(
            f"Processing... {current_progress[0]}/{total_grains} grains"
        )
    ...
    try:
        ...
        st.session_state.simulation_results["original_data"] = st.session_state.data_file
...
def main():
    ...
    st.title("PG NanoSIMS Simulations v3.0")
    st.markdown("---")
